...
if __name__ == "__main__":
    logger.info(f"Starting 5G Core Deployment Controller on http://0.0.0.0:{API_PORT}")
    logger.info(f"Log level set to: {LOG_LEVEL}")
    # En desarrollo desde raíz del proyecto, usa la ruta completa del módulo
    uvicorn.run("src.5g_core_deployment_controller.main:app", host="0.0.0.0", port=API_PORT, reload=True)